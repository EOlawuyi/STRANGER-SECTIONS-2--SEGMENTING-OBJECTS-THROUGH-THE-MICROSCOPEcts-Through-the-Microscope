...
ImageColor = ["r", "r", "r", "r", "r", "r", "r", "r", "r", "r", "r", "r",
...
#      for j in range(0,b):
 ...
 #  img2.show()
   ...
   for i in range(0,a1):
      for j in range(0,b1):
         if((datae[i,j,0] > 91) & (datae[i,j,0] < 133)):
            if((datae[i,j,1] > 92) & (datae[i,j,1] < 134)):
               if( ((datae[i,j,2] > 97) & (datae[i,j,2] < 139))):
   ...
   for i in range(0,a1):
      for j in range(0,b1):
         if((datae[i,j,0] > 167) & (datae[i,j,0] < 185)):
            if((datae[i,j,1] > 164) & (datae[i,j,1] < 182)):
               if( ((datae[i,j,2] > 163) & (datae[i,j,2] < 181))):
                  #THRESHOLD LIGHT GRAY COLOR  [176 173 172]
   ...
   for i in range(0,a1):
      for j in range(0,b1):
         if((datae[i,j,0] > 112) & (datae[i,j,0] < 184)):
            if((datae[i,j,1] > 122) & (datae[i,j,1] < 194)):
               if( ((datae[i,j,2] > 97) & (datae[i,j,2] < 169))):
                  #THRESHOLD LIGHT GRAY COLOR 3 [211 211 211] 4 CHOSEN - [148 158 133]
                  lightgray3 = lightgray3 + 1
                  datad[i,j,0] = 255
                  datad[i,j,1] = 0
                  datad[i,j,2] = 0
   ...
   img8bb= asarray(graytest3)
   img8.show()
   print("graytest3")
   print(graytest3.shape)
   print("First Image 8 size")
   print(img8.size)
   print("First Image 8 asarray")
   print(img8bb.shape)
   for m in range(0,linecounttest+1):
      ...
      #img8.putpixel((int(y.values[int(gray1b[j]),1][0,0]),int(y.values[int(gray1b[j]),1][0,1])), 200)
         #img8bb.putpixel((int(y.values[int(gray1b[m]),1][p,1]),int(y.values[int(gray1b[m]),1][p,0])), 255)
         img8bb[int(y.values[int(gray1b[m]),1][p,0]),int(y.values[int(gray1b[m]),1][p,1])] =  255

         #print("starting 2")

   #print(y.values[29,1][0,0])
   print("Final Image for Liptinite - Dark Brown:")
  # gray_arr = np.dot(img8bb[...,:3],[0.2989,0.5870,0.1140])
  # print(gray_arr.shape)
  # plt.imshow(gray_arr, cmap='gray')
   #plt.show()
  
  #img8bbb = im.fromarray(img8bb)
  #img8bbb.show()
  # print("Second Image 8 size")
  # print(img8bbb.size)
   ...
   gray_arr = np.dot(datak[...,:3],[0.2989,0.5870,0.1140])
   #plt.imshow(gray_arr, cmap=plt.get_cmap('gray'))
   #plt.imshow(gray_arr, cmap='gray')
   #plt.show()

   LiptiniteBrown = img8bb
   [a1,b1] = LiptiniteBrown.shape
   FinalImageMacerel = np.zeros((d,e))
   FinalImageMacerelb = np.zeros((d,e))
   FinalImageMacerel2 = img8bb
   print(a1)
   print(b1)

   for i in range(0,d):
      for j in range(0,e):
         if((LiptiniteBrown[i,j] != 0)):
            FinalImageMacerel2[i,j]= 255
            FinalImageMacerel[i,j] = 255
         else:
            FinalImageMacerel2[i,j] = 0
            #get value inserted
   print("Final Macerel Image 2:")           
   #imgfinal = im.fromarray(FinalImageMacerel, 'RGB')
   #FinalImageMacerel2.show()
   #gray_arr2 = np.dot(FinalImageMacerel2[...,:3],[0.2989,0.5870,0.1140])
   # plt.imshow(gray_arr2, cmap=plt.get_cmap('gray'))
   plt.imshow(FinalImageMacerel2, cmap='gray')  
  #plt.show()
   #FinalImageMacerel2b= asarray(FinalImageMacerel2)
   # img334 = im.fromarray(FinalImageMacerel2b)
   # img334.show()
   ...
   for i in range(0,d-1):
      for j in range(0,e-1):
         if((FinalImageMacerel2[i,j] != 0)):
            FinalImageMacerelb[i,j] = 2
            gray_arr2[i,j] = 2
            #get value inserted
   plt.imshow(gray_arr2, cmap='gray')
   #plt.show()  
   np.save(Imagefile3[z],gray_arr2)

   
   colorthresholdcount = [orange, yellow, brown, gray, darkgray, lightgray, lightgray3]
   ...
   if(chosencolorcount == darkgray):
      print("Dark Gray!")
      ImageColor[z] = "Dark Gray"
      for i in range(0,a1-1):
         for j in range(0,b1-1):
            if((datae[i,j,0] > 43) & (datae[i,j,0] < 93)):
               if((datae[i,j,1] > 47) & (datae[i,j,1] < 95)):
                  if( ((datae[i,j,2] > 60) & (datae[i,j,2] < 108))):
                     #THRESHOLD DARK GRAY COLOR  [69 71 84]
                     datad[i,j,0] = 255
                     datad[i,j,1] = 0
                     datad[i,j,2] = 0
   ...
   if(chosencolorcount == lightgray):
      print("Light Gray!")
      ImageColor[z] = "Light Gray"
      for i in range(0,a1-1):
         for j in range(0,b1-1):
            if((datae[i,j,0] > 167) & (datae[i,j,0] < 185)):
               if((datae[i,j,1] > 164) & (datae[i,j,1] < 182)):
   ...
   if(chosencolorcount == lightgray3):
      print("Light Gray 3!")
      ImageColor[z] = "Light Gray 3"
      for i in range(0,a1-1):
         for j in range(0,b1-1):
            if((datae[i,j,0] > 112) & (datae[i,j,0] < 184)):
               if((datae[i,j,1] > 122) & (datae[i,j,1] < 194)):
  ...
  # print(img8bbb.size)
   ...
   plt.imshow(gray_arr2, cmap='gray')
   #plt.show()  
   np.save(Imagefile4[z],gray_arr2)
